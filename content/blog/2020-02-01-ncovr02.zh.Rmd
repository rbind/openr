...
```{r, fig.cap='网易的全国新型冠状病毒疫情地图。'}
knitr::include_graphics('img/map-ncov-netease.png')
```
...
点击左上角的缩放键，就可以 zoom in/out 了。点击地图上任一位置，就会弹出对应信息（当然，如果你是在微信公众号读到了本文，是无法进行交互的。请点击文末左下角的“阅读原文”，移步网页版）。
...
1. 代表确诊病例颜色的深浅是线性的。湖北省颜色最深，而其他省之间的差异不太明显。这是由于各省确证病例数差别很大而导致的。
...
本节里涉及的代码虽然结构简单，然而终究有点长，不适合懒人。要是想每天关注动态，可以用我打包封装在 "ncovr" 包里好的函数 `plo_map`。忘了上面的代码吧，因为用下面的代码更方便：
...
```{r}
plot_map(ncov$area, filter = '待明确地区', color = "YlGnBl")
```

除了本文介绍的方法，绘制地图的方法还有很多，将来有机会咱们再逐个介绍。
